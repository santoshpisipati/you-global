...
namespace GlobalPanda.DataProviders
{
    public class ConsultantDataProvider
    {
        ...
        public static string getJobcodePrefix(int consultantId)
        {
            string jobcode = string.Empty;
            string sql = "select jobcode from consultants where consultantid=?consultantid ";
            jobcode = DAO.ExecuteScalar(sql, new MySqlParameter("consultantId", consultantId)).ToString();
            return jobcode;
        }
